...
def setup_request(uri_triple):
    uri, depth, seed = uri_triple
    if depth == 0:
        dbconnector.insert_crawl_seed(uri, crawlid)
        dbconnector.commit()
    response = make_request(uri, depth, seed)
    return response

def worker_function(w, uri_queue):
    print("Started Worker {}.".format(w))
    while not uri_queue.empty():
        uri = uri_queue.get()
        resp = setup_request(uri)
        if isinstance(resp, list):
            [uri_queue.put(child_uri) for child_uri in resp]
    raise SystemExit(0)
...
if __name__ == '__main__':
    ...
    uri_queue = mp.Queue()
    [uri_queue.put(uri) for uri in initial_URI_list]
    worker_array = []
    for i in range(workers):
        worker = mp.Process(target=worker_function, args=(i, uri_queue))
        worker_array.append(worker)
    [worker.start() for worker in worker_array]
